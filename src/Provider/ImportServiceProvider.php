...
/**
 * Registers the three `import:*` commands shipped by WP06.
 *
 * Collaborator bindings ({@see MigrationRunner}, {@see MigrationRegistry},
 * {@see MigrationIdMap}) live in the migration package's own
 * {@see \Waaseyaa\Migration\ServiceProvider}; this provider only binds the
 * thin command handler classes (which the CLI kernel container resolves via
 * `[Class, method]` handler references) and yields the {@see CommandDefinition}s.
 */
final class ImportServiceProvider extends ServiceProvider implements HasNativeCommandsInterface
{
    public function register(): void
    {
        ...
        $this->singleton(ImportStatusCommand::class, function (): ImportStatusCommand {
            ...
            return new ImportStatusCommand($registry, $idMap);
        });
    }
