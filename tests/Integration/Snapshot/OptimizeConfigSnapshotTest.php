...
/**
 * Snapshot test: verifies optimize:config --help output matches the WP01 fixture byte-for-byte.
 */
#[CoversNothing]
final class OptimizeConfigSnapshotTest extends TestCase
{
    ...
    public function helpOutputMatchesFixtureByteForByte(): void
    {
        $fixture = file_get_contents(
            __DIR__ . '/../../Fixtures/snapshots/optimize__config.help.stdout',
        );
        self::assertNotFalse($fixture, 'Fixture file must exist');

        [$stdout, $exitCode] = $this->runHelp('optimize:config');

        self::assertSame(0, $exitCode, 'optimize:config --help must exit 0');
        self::assertSame($fixture, $stdout, 'optimize:config --help output must match fixture byte-for-byte');
    }
