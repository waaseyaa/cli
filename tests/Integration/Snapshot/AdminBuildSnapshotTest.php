...
/**
 * Snapshot test: verifies admin:build --help output matches the fixture byte-for-byte.
 */
#[CoversNothing]
final class AdminBuildSnapshotTest extends TestCase
{
    ...
    public function helpOutputMatchesFixtureByteForByte(): void
    {
        $fixture = file_get_contents(
            __DIR__ . '/../../Fixtures/snapshots/admin__build.help.stdout',
        );
        self::assertNotFalse($fixture, 'Fixture file must exist');

        [$stdout, $exitCode] = $this->runHelp('admin:build');

        self::assertSame(0, $exitCode, 'admin:build --help must exit 0');
        self::assertSame($fixture, $stdout, 'admin:build --help output must match fixture byte-for-byte');
    }
