...
/**
 * Snapshot test: verifies migrate --help output matches the fixture byte-for-byte.
 *
 * A passing test here implies:
 *   diff <(bin/waaseyaa migrate --help) packages/cli/tests/Fixtures/snapshots/migrate.help.stdout
 * exits 0.
 */
#[CoversNothing]
final class MigrateSnapshotTest extends TestCase
{
    ...
    public function helpOutputMatchesFixtureByteForByte(): void
    {
        $fixture = file_get_contents(
            __DIR__ . '/../../Fixtures/snapshots/migrate.help.stdout',
        );
        self::assertNotFalse($fixture, 'Fixture file must exist');

        [$stdout, $exitCode] = $this->runHelp('migrate');

        self::assertSame(0, $exitCode, 'migrate --help must exit 0');
        self::assertSame($fixture, $stdout, 'migrate --help output must match fixture byte-for-byte');
    }
