...
/**
 * Snapshot test: verifies schedule:list --help output matches the pseudo-baseline fixture byte-for-byte.
 */
#[CoversNothing]
final class ScheduleListSnapshotTest extends TestCase
{
    ...
    public function helpOutputMatchesFixtureByteForByte(): void
    {
        $fixture = file_get_contents(
            __DIR__ . '/../../Fixtures/snapshots/schedule__list.help.stdout',
        );
        self::assertNotFalse($fixture, 'Fixture file must exist');

        [$stdout, $exitCode] = $this->runHelp('schedule:list');

        self::assertSame(0, $exitCode, 'schedule:list --help must exit 0');
        self::assertSame($fixture, $stdout, 'schedule:list --help output must match fixture byte-for-byte');
    }
