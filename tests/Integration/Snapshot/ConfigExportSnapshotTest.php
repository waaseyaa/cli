...
/**
 * Snapshot test: verifies config:export --help output matches the fixture byte-for-byte.
 */
#[CoversNothing]
final class ConfigExportSnapshotTest extends TestCase
{
    ...
    public function helpOutputMatchesFixtureByteForByte(): void
    {
        $fixture = file_get_contents(
            __DIR__ . '/../../Fixtures/snapshots/config__export.help.stdout',
        );
        self::assertNotFalse($fixture, 'Fixture file must exist');

        [$stdout, $exitCode] = $this->runHelp('config:export');

        self::assertSame(0, $exitCode, 'config:export --help must exit 0');
        self::assertSame($fixture, $stdout, 'config:export --help output must match fixture byte-for-byte');
    }
