...
/**
 * Snapshot test: verifies make:job --help output matches the fixture byte-for-byte.
 */
#[CoversNothing]
final class MakeJobSnapshotTest extends TestCase
{
    ...
    public function helpOutputMatchesFixtureByteForByte(): void
    {
        $fixture = file_get_contents(
            __DIR__ . '/../../Fixtures/snapshots/make__job.help.stdout',
        );
        self::assertNotFalse($fixture, 'Fixture file must exist');

        [$stdout, $exitCode] = $this->runHelp('make:job');

        self::assertSame(0, $exitCode, 'make:job --help must exit 0');
        self::assertSame($fixture, $stdout, 'make:job --help output must match fixture byte-for-byte');
    }
