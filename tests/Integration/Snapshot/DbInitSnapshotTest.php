...
/**
 * Snapshot test: verifies db:init --help output matches the fixture byte-for-byte.
 */
#[CoversNothing]
final class DbInitSnapshotTest extends TestCase
{
    ...
    public function helpOutputMatchesFixtureByteForByte(): void
    {
        $fixture = file_get_contents(
            __DIR__ . '/../../Fixtures/snapshots/db__init.help.stdout',
        );
        self::assertNotFalse($fixture, 'Fixture file must exist');

        [$stdout, $exitCode] = $this->runHelp('db:init');

        self::assertSame(0, $exitCode, 'db:init --help must exit 0');
        self::assertSame($fixture, $stdout, 'db:init --help output must match fixture byte-for-byte');
    }
