...
/**
 * Snapshot test: verifies make:listener --help output matches the fixture byte-for-byte.
 */
#[CoversNothing]
final class MakeListenerSnapshotTest extends TestCase
{
    ...
    public function helpOutputMatchesFixtureByteForByte(): void
    {
        $fixture = file_get_contents(
            __DIR__ . '/../../Fixtures/snapshots/make__listener.help.stdout',
        );
        self::assertNotFalse($fixture, 'Fixture file must exist');

        [$stdout, $exitCode] = $this->runHelp('make:listener');

        self::assertSame(0, $exitCode, 'make:listener --help must exit 0');
        self::assertSame($fixture, $stdout, 'make:listener --help output must match fixture byte-for-byte');
    }
