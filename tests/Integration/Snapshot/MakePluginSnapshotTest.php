...
/**
 * Snapshot test: verifies make:plugin --help output matches the fixture byte-for-byte.
 */
#[CoversNothing]
final class MakePluginSnapshotTest extends TestCase
{
    ...
    public function helpOutputMatchesFixtureByteForByte(): void
    {
        $fixture = file_get_contents(
            __DIR__ . '/../../Fixtures/snapshots/make__plugin.help.stdout',
        );
        self::assertNotFalse($fixture, 'Fixture file must exist');

        [$stdout, $exitCode] = $this->runHelp('make:plugin');

        self::assertSame(0, $exitCode, 'make:plugin --help must exit 0');
        self::assertSame($fixture, $stdout, 'make:plugin --help output must match fixture byte-for-byte');
    }
