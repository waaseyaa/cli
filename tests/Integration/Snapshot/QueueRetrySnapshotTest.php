...
/**
 * Snapshot test: verifies queue:retry --help output matches the WP01 fixture byte-for-byte.
 */
#[CoversNothing]
final class QueueRetrySnapshotTest extends TestCase
{
    ...
    public function helpOutputMatchesFixtureByteForByte(): void
    {
        $fixture = file_get_contents(
            __DIR__ . '/../../Fixtures/snapshots/queue__retry.help.stdout',
        );
        self::assertNotFalse($fixture, 'Fixture file must exist');

        [$stdout, $exitCode] = $this->runHelp('queue:retry');

        self::assertSame(0, $exitCode, 'queue:retry --help must exit 0');
        self::assertSame($fixture, $stdout, 'queue:retry --help output must match fixture byte-for-byte');
    }
