...
/**
 * Snapshot test: verifies perf:compare --help output matches the WP01 fixture byte-for-byte.
 */
#[CoversNothing]
final class PerfCompareSnapshotTest extends TestCase
{
    ...
    public function helpOutputMatchesFixtureByteForByte(): void
    {
        $fixture = file_get_contents(
            __DIR__ . '/../../Fixtures/snapshots/perf__compare.help.stdout',
        );
        self::assertNotFalse($fixture, 'Fixture file must exist');

        [$stdout, $exitCode] = $this->runHelp('perf:compare');

        self::assertSame(0, $exitCode, 'perf:compare --help must exit 0');
        self::assertSame($fixture, $stdout, 'perf:compare --help output must match fixture byte-for-byte');
    }
