...
/**
 * Snapshot test: verifies queue:flush --help output matches the WP01 fixture byte-for-byte.
 */
#[CoversNothing]
final class QueueFlushSnapshotTest extends TestCase
{
    ...
    public function helpOutputMatchesFixtureByteForByte(): void
    {
        $fixture = file_get_contents(
            __DIR__ . '/../../Fixtures/snapshots/queue__flush.help.stdout',
        );
        self::assertNotFalse($fixture, 'Fixture file must exist');

        [$stdout, $exitCode] = $this->runHelp('queue:flush');

        self::assertSame(0, $exitCode, 'queue:flush --help must exit 0');
        self::assertSame($fixture, $stdout, 'queue:flush --help output must match fixture byte-for-byte');
    }
