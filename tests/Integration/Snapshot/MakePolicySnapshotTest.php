...
/**
 * Snapshot test: verifies make:policy --help output matches the fixture byte-for-byte.
 */
#[CoversNothing]
final class MakePolicySnapshotTest extends TestCase
{
    ...
    public function helpOutputMatchesFixtureByteForByte(): void
    {
        $fixture = file_get_contents(
            __DIR__ . '/../../Fixtures/snapshots/make__policy.help.stdout',
        );
        self::assertNotFalse($fixture, 'Fixture file must exist');

        [$stdout, $exitCode] = $this->runHelp('make:policy');

        self::assertSame(0, $exitCode, 'make:policy --help must exit 0');
        self::assertSame($fixture, $stdout, 'make:policy --help output must match fixture byte-for-byte');
    }
