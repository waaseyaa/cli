...
/**
 * Snapshot test: verifies telescope:validate --help output matches the pseudo-baseline fixture byte-for-byte.
 *
 * Note: No WP01 baseline exists for telescope:validate — this is a pseudo-baseline generated at port time.
 */
#[CoversNothing]
final class TelescopeValidateSnapshotTest extends TestCase
{
    ...
    public function helpOutputMatchesFixtureByteForByte(): void
    {
        $fixture = file_get_contents(
            __DIR__ . '/../../Fixtures/snapshots/telescope__validate.help.stdout',
        );
        self::assertNotFalse($fixture, 'Fixture file must exist');

        [$stdout, $exitCode] = $this->runHelp('telescope:validate');

        self::assertSame(0, $exitCode, 'telescope:validate --help must exit 0');
        self::assertSame($fixture, $stdout, 'telescope:validate --help output must match fixture byte-for-byte');
    }
