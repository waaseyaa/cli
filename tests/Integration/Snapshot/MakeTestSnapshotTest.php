...
/**
 * Snapshot test: verifies make:test --help output matches the fixture byte-for-byte.
 */
#[CoversNothing]
final class MakeTestSnapshotTest extends TestCase
{
    ...
    public function helpOutputMatchesFixtureByteForByte(): void
    {
        $fixture = file_get_contents(
            __DIR__ . '/../../Fixtures/snapshots/make__test.help.stdout',
        );
        self::assertNotFalse($fixture, 'Fixture file must exist');

        [$stdout, $exitCode] = $this->runHelp('make:test');

        self::assertSame(0, $exitCode, 'make:test --help must exit 0');
        self::assertSame($fixture, $stdout, 'make:test --help output must match fixture byte-for-byte');
    }
