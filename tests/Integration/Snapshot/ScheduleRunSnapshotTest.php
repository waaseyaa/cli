...
/**
 * Snapshot test: verifies schedule:run --help output matches the pseudo-baseline fixture byte-for-byte.
 */
#[CoversNothing]
final class ScheduleRunSnapshotTest extends TestCase
{
    ...
    public function helpOutputMatchesFixtureByteForByte(): void
    {
        $fixture = file_get_contents(
            __DIR__ . '/../../Fixtures/snapshots/schedule__run.help.stdout',
        );
        self::assertNotFalse($fixture, 'Fixture file must exist');

        [$stdout, $exitCode] = $this->runHelp('schedule:run');

        self::assertSame(0, $exitCode, 'schedule:run --help must exit 0');
        self::assertSame($fixture, $stdout, 'schedule:run --help output must match fixture byte-for-byte');
    }
