...
/**
 * Snapshot test: verifies audit:log --help output matches the fixture byte-for-byte.
 */
#[CoversNothing]
final class AuditLogSnapshotTest extends TestCase
{
    ...
    public function helpOutputMatchesFixtureByteForByte(): void
    {
        $fixture = file_get_contents(
            __DIR__ . '/../../Fixtures/snapshots/audit__log.help.stdout',
        );
        self::assertNotFalse($fixture, 'Fixture file must exist');

        [$stdout, $exitCode] = $this->runHelp('audit:log');

        self::assertSame(0, $exitCode, 'audit:log --help must exit 0');
        self::assertSame($fixture, $stdout, 'audit:log --help output must match fixture byte-for-byte');
    }
