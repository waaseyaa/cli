...
/**
 * Snapshot test: verifies queue:work --help output matches the WP01 fixture byte-for-byte.
 */
#[CoversNothing]
final class QueueWorkSnapshotTest extends TestCase
{
    ...
    public function helpOutputMatchesFixtureByteForByte(): void
    {
        $fixture = file_get_contents(
            __DIR__ . '/../../Fixtures/snapshots/queue__work.help.stdout',
        );
        self::assertNotFalse($fixture, 'Fixture file must exist');

        [$stdout, $exitCode] = $this->runHelp('queue:work');

        self::assertSame(0, $exitCode, 'queue:work --help must exit 0');
        self::assertSame($fixture, $stdout, 'queue:work --help output must match fixture byte-for-byte');
    }
