...
/**
 * Snapshot test: verifies cache:clear --help output matches the fixture byte-for-byte.
 */
#[CoversNothing]
final class CacheClearSnapshotTest extends TestCase
{
    ...
    public function helpOutputMatchesFixtureByteForByte(): void
    {
        $fixture = file_get_contents(
            __DIR__ . '/../../Fixtures/snapshots/cache__clear.help.stdout',
        );
        self::assertNotFalse($fixture, 'Fixture file must exist');

        [$stdout, $exitCode] = $this->runHelp('cache:clear');

        self::assertSame(0, $exitCode, 'cache:clear --help must exit 0');
        self::assertSame($fixture, $stdout, 'cache:clear --help output must match fixture byte-for-byte');
    }
