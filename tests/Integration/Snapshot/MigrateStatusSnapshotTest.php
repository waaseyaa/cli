...
/**
 * Snapshot test: verifies migrate:status --help output matches the fixture byte-for-byte.
 *
 * A passing test here implies:
 *   diff <(bin/waaseyaa migrate:status --help) packages/cli/tests/Fixtures/snapshots/migrate__status.help.stdout
 * exits 0.
 */
#[CoversNothing]
final class MigrateStatusSnapshotTest extends TestCase
{
    ...
    public function helpOutputMatchesFixtureByteForByte(): void
    {
        $fixture = file_get_contents(
            __DIR__ . '/../../Fixtures/snapshots/migrate__status.help.stdout',
        );
        self::assertNotFalse($fixture, 'Fixture file must exist');

        [$stdout, $exitCode] = $this->runHelp('migrate:status');

        self::assertSame(0, $exitCode, 'migrate:status --help must exit 0');
        self::assertSame($fixture, $stdout, 'migrate:status --help output must match fixture byte-for-byte');
    }
