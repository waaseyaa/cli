...
/**
 * Snapshot test: verifies optimize:clear --help output matches the WP01 fixture byte-for-byte.
 */
#[CoversNothing]
final class OptimizeClearSnapshotTest extends TestCase
{
    ...
    public function helpOutputMatchesFixtureByteForByte(): void
    {
        $fixture = file_get_contents(
            __DIR__ . '/../../Fixtures/snapshots/optimize__clear.help.stdout',
        );
        self::assertNotFalse($fixture, 'Fixture file must exist');

        [$stdout, $exitCode] = $this->runHelp('optimize:clear');

        self::assertSame(0, $exitCode, 'optimize:clear --help must exit 0');
        self::assertSame($fixture, $stdout, 'optimize:clear --help output must match fixture byte-for-byte');
    }
